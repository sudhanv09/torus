  ...
  connectedCallback() {
    this.#t ||
      (document.readyState === "loading"
        ? document.addEventListener("DOMContentLoaded", () => this.#e(), {
            once: !0,
          })
        : this.#e());
  }
...
(typeof window < "u" && (window.OtBase = OtBase),
  "commandForElement" in HTMLButtonElement.prototype ||
    document.addEventListener("click", (n) => {
      const t = n.target.closest("[commandfor]");
      if (!t) return;
      const e = document.getElementById(t.getAttribute("commandfor"));
      if (!e) return;
...
class OtTabs extends OtBase {
  ...
  init() {
    ...
    this.#t.forEach((i, o) => {
      const s = this.#e[o];
      if (!s) return;
      const r = i.id || `ot-tab-${this.uid()}`,
        a = s.id || `ot-panel-${this.uid()}`;
      ((i.id = r),
        (s.id = a),
        i.setAttribute("aria-controls", a),
        s.setAttribute("aria-labelledby", r),
        i.addEventListener("click", this),
        i.addEventListener("keydown", this));
    });
    const e = this.#t.findIndex((i) => i.ariaSelected === "true");
    this.#n(e >= 0 ? e : 0);
  }
  onclick(t) {
    const e = this.#t.indexOf(t.target.closest('[role="tab"]'));
    e >= 0 && this.#n(e);
  }
  onkeydown(t) {
    const { key: e } = t,
    ...
    switch (e) {
      case "ArrowLeft":
        (t.preventDefault(), (o = i - 1), o < 0 && (o = this.#t.length - 1));
        break;
      case "ArrowRight":
        (t.preventDefault(), (o = (i + 1) % this.#t.length));
        break;
      default:
        return;
    }
    (this.#n(o), this.#t[o].focus());
  }
  #n(t) {
    (this.#t.forEach((e, i) => {
      const o = i === t;
    ...
    ((this.#t = this.$("[popover]")),
      ...
      !(!this.#t || !this.#e) &&
        ...
        (this.#n = () => {
          ...
          ((this.#t.style.top = `${t.bottom}px`),
            (this.#t.style.left = `${t.left}px`));
        })));
  }
  ontoggle(t) {
    t.newState === "open"
      ? (this.#n(),
        window.addEventListener("scroll", this.#n, !0),
        this.$('[role="menuitem"]')?.focus(),
        (this.#e.ariaExpanded = "true"))
      : (window.removeEventListener("scroll", this.#n, !0),
        (this.#e.ariaExpanded = "false"),
        this.#e.focus());
  }
  onkeydown(t) {
    if (!t.target.matches('[role="menuitem"]')) return;
    const e = this.$$('[role="menuitem"]'),
    ...
    switch (t.key) {
      case "ArrowDown":
        ...
        break;
      case "ArrowUp":
        (t.preventDefault(), e[i - 1 < 0 ? e.length - 1 : i - 1]?.focus());
        break;
    }
  }
  cleanup() {
...
function getContainer(n) {
  if (!containers[n]) {
    ...
    ((t.className = "toast-container"),
      ...
      t.setAttribute("data-placement", n),
...
function show(n, t = {}) {
  const { placement: e = DEFAULT_PLACEMENT, duration: i = DEFAULT_DURATION } =
      t,
    o = getContainer(e);
  n.classList.add("toast");
  let s;
  return (
    (n.onmouseenter = () => clearTimeout(s)),
    (n.onmouseleave = () => {
      i > 0 && (s = setTimeout(() => removeToast(n, o), i));
    }),
    n.setAttribute("data-entering", ""),
    o.appendChild(n),
    o.showPopover(),
...
((ot.toast = function (n, t, e = {}) {
  const { variant: i = "info", ...o } = e,
    s = document.createElement("output");
  if ((s.setAttribute("data-variant", i), t)) {
    const r = document.createElement("h6");
    ((r.className = "toast-title"),
      (r.style.color = `var(--${i})`),
      (r.textContent = t),
      s.appendChild(r));
  }
  if (n) {
    ...
    ((r.className = "toast-message"), (r.textContent = n), s.appendChild(r));
  }
  return show(s, o);
}),
  (ot.toastEl = function (n, t = {}) {
    let e;
    return (
      n instanceof HTMLTemplateElement
        ? (e = n.content.firstElementChild.cloneNode(!0))
        : typeof n == "string"
          ? (e = document.querySelector(n).cloneNode(!0))
          : (e = n.cloneNode(!0)),
      e.removeAttribute("id"),
      show(e, t)
    );
  }));
function removeToast(n, t) {
  if (n.hasAttribute("data-exiting")) return;
  n.setAttribute("data-exiting", "");
  const e = () => {
    (n.remove(), t.children.length || t.hidePopover());
  };
  (n.addEventListener("transitionend", e, { once: !0 }), setTimeout(e, 200));
}
((ot.toast.clear = function (n) {
  n && containers[n]
    ? ((containers[n].innerHTML = ""), containers[n].hidePopover())
    : Object.values(containers).forEach((t) => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll("[title]").forEach((n) => {
      ...
      t &&
        (n.setAttribute("data-tooltip", t),
        n.hasAttribute("aria-label") || n.setAttribute("aria-label", t),
        n.removeAttribute("title"));
    });
  }),
  document.addEventListener("click", (n) => {
    const t = n.target.closest("[data-sidebar-toggle]");
    t &&
